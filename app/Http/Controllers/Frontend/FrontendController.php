...
class FrontendController extends Controller
{
    ...
    public function categoria()
    {
        ...
        $novaImobiliario = produto::latest()->take(16)->get();
        $novaMobiliario = imobiliario::latest()->take(16)->get();
        return view('frontend.colecao.categorias.index', compact('categoria','novaImobiliario','novaMobiliario'));
    }
    ...
    public function produto($categoria_nome)
    {
       ...
       if($categoria){
            ...
            $imobiliario = $categoria->imobiliario()->get();
            return view('frontend.colecao.produtos.index', compact('produto','categoria','imobiliario','novaImobiliario','novaMobiliario'));
       }else{
    ...
    public function produtoView(string $categoria_nome, string $produto_nome)
    {
        ...
        if($categoria){
             ...
             $imobiliario = $categoria->imobiliario()->where('nome',$produto_nome)->where('estado','0')->first();
             if($produto){
                return view('frontend.colecao.produtos.view', compact('produto','categoria','novaImobiliario'));
             }
             elseif($imobiliario){
                return view('frontend.colecao.produtos.ver', compact('imobiliario','categoria','novaMobiliario'));
             }
             else{
                return redirect()-back();
             }
    ...
    public function novidade()
    {
        ...
        $novaMobiliario = imobiliario::latest()->take(16)->get();
        return view('frontend.pages.novidade', compact('sliders','novaImobiliario','categoria','novaMobiliario'));
    }
